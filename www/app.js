...
/* ==========================================================================
   GESTION DU RENDU DE L'IA EN TEMPS RÉEL (STREAMING UI)
   ========================================================================== */

async function lancerCoursInteractif() {
    const inputSujet = document.getElementById("course-topic");
    const containerExplanation = document.getElementById("explanation-section");
    const outputCard = document.getElementById("output-card");
    const sujet = inputSujet.value.trim();

    if (!sujet) {
        alert("⚠️ Veuillez saisir une notion à étudier (Ex: Équations différentielles) !");
        return;
    }

    // 1. Préparation visuelle immédiate (Zéro latence perçue)
    outputCard.style.display = "block";
    containerExplanation.innerHTML = `
        <div class="content-box">
            <h2 style="color: #4f46e5; margin-top: 0;">📘 ${sujet}</h2>
            <div id="ai-streaming-output" class="streaming-cursor" style="white-space: pre-wrap; line-height: 1.8;"></div>
        </div>
    `;

    // Fait défiler l'écran vers la zone de cours en douceur
    outputCard.scrollIntoView({ behavior: "smooth", block: "start" });

    const outputElement = document.getElementById("ai-streaming-output");
    
    // Prompt pédagogique strict pour le contexte LMD ivoirien
    const promptPedagogique = `Tu es un professeur d'université expert dans le système LMD en Côte d'Ivoire (UFHB / UNA). 
    Fais un cours clair, structuré avec des exemples concrets et un exercice corrigé sur : "${sujet}". 
    Utilise un ton encourageant, rigoureux et adapte-toi à un étudiant d'université.`;

    try {
        // 2. Lancement du streaming avec fonction de rappel (Callback)
        await genererCoursStream(promptPedagogique, (fragmentReçu, texteAccumule) => {
            // On injecte le nouveau texte directement dans le DOM
            outputElement.innerText = texteAccumule;

            // Optionnel : Auto-Scroll doux pour suivre la lecture sur smartphone
            const basDePage = outputElement.getBoundingClientRect().bottom;
            if (basDePage > window.innerHeight - 100) {
                window.scrollBy({ top: 40, behavior: "instant" });
            }
        });

        // 3. Fin du streaming : On retire le curseur clignotant
        outputElement.classList.remove("streaming-cursor");
        
        // C'est le moment idéal pour appliquer un formatage Markdown (si vous utilisez Marked.js)
        // outputElement.innerHTML = marked.parse(outputElement.innerText);

    } catch (erreur) {
        outputElement.classList.remove("streaming-cursor");
        outputElement.innerHTML += `\n\n<p style="color: #ef4444; font-weight: bold;">❌ Une interruption de réseau est survenue. Veuillez réessayer.</p>`;
    }
}

// Connexion de l'événement au bouton "Lancer le Cours"
document.addEventListener("DOMContentLoaded", () => {
    const btnLearn = document.getElementById("learn-btn");
    if (btnLearn) {
        btnLearn.addEventListener("click", lancerCoursInteractif);
    }
});